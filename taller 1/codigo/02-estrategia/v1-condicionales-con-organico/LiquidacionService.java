...
public class LiquidacionService {
    ...
    public BigDecimal liquidar(Muestra m) {
        ...
        } else if (m.tipo().equals("ORGANICO")) {
            // El esquema del pergamino, pero con piso de 0,90 y prima de 0,06.
            BigDecimal exceso = m.humedad().subtract(new BigDecimal("11"));
            factor = BigDecimal.ONE.subtract(exceso.multiply(new BigDecimal("0.02")))
                    .setScale(4, RoundingMode.HALF_UP)
                    .max(new BigDecimal("0.9000"));
            if (m.certificada()) {
                factor = factor.add(new BigDecimal("0.0600")).min(new BigDecimal("1.1000"));
            }
